...
class PathGUI:
    def __init__(self, ros_node):
        ...
        self.last_drawn = []
        self.selected_indices = []  # Publish 시 표시할 좌표 인덱스
        self.n_domino = None        # Publish 시 표시할 쌓을 개수
        self._poll_path()
    ...
    def draw_path(self, path):
        ...
        if max(ys) - min(ys) > 1e-5:
            ...
            scale_y = 1.0
        def tx(x): return pad + int((x - min(xs)) * scale_x)
        def ty(y): return H - pad - int((y - min(ys)) * scale_y)
        for i, (x, y) in enumerate(path):
            cx, cy = tx(x), ty(y)
            # 선택된 인덱스면 빨간색/아래에 개수 표시
    ...
    def on_publish(self):
        # 좌표 인덱스와 개수 읽기 (messagebox 사용하지 않고, 잘못된 입력시 그냥 return)
        try:
